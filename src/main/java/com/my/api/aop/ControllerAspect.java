...
public class ControllerAspect {
     ...
     */
    @Pointcut("execution(* com.my.api.controller..*(..))")
...
//                sb.append("請求參數(入參):" + Arrays.toString(joinPoint.getArgs()));
    ...
    public void doAfterThrowing(JoinPoint joinPoint, Exception e){
        String clazzName = joinPoint.getSignature().getDeclaringTypeName();
        String methodName = joinPoint.getSignature().getName();
        logger.error("[" + clazzName + "] - [" + methodName + "] (2) 執行失敗, 執行" + joinPoint.getSignature().getDeclaringTypeName() + "." + joinPoint.getSignature().getName() + " <-- 執行過程中拋出 excepton: " + e );
    }
